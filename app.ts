...
function setSessionCookie(req: express.Request, res: express.Response, token: string) {
  ...
  res.setHeader(
    "Set-Cookie",
    `${SESSION_COOKIE}=${token}; HttpOnly; Path=/; Max-Age=${30 * 24 * 60 * 60}; SameSite=Lax${isSecure ? "; Secure" : ""}`
  );
}
...
async function requireSession(req: express.Request): ReturnType<typeof getSession> {
        ...
        <body>
          ...
          <script>
            ...
            if (window.opener) {
              ...
              localStorage.setItem("discord_officer_user", JSON.stringify(userData));
...
// Farm account linking
app.get("/api/farms/:playerId", async (req, res) => {
  try {
    const farms = await getFarmLinks(req.params.playerId);
    res.json({ farms });
  } catch (err) {
